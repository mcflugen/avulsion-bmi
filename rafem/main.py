...
def main():
    ...
    parser.add_argument("--days", type=int, default=0, help="Run model for DAYS")
    parser.add_argument("--years", type=int, default=0, help="Run model for YEARS")
    parser.add_argument(
        "--plot_elev", action="store_true", help="Plot final elevations"
    )
    parser.add_argument("--plot_prof", action="store_true", help="Plot final profile")
    ...
    parser.add_argument("--runID", type=int, default=1, help="Experiment ID number")
    ...
    for k in range(n_steps):
        ...
        if args.save & (k % args.spacing == 0):
            ...
            np.savetxt(
                ...
                + str(args.runID)
                + "/elev_"
                + str(k * avulsion.get_time_step() / 365)
                + ".out",
                z,
                fmt="%.5f",
            )
    ...
    if args.save:
        ...
        if np.sum(avul_info) > 0:
            np.savetxt(
                "run" + str(args.runID) + "/avulsions" + str(args.runID),
                avul_info,
                fmt="%i %.2f %i",
            )
